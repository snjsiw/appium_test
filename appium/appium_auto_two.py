...
def stop_appium(post_nums=[4723, 4726]):
    '''关闭Appium服务'''
    ...
    for post_num in post_nums:
        if pc.upper() == 'WIN':
            ...
            if p0 != '' and 'LISTENING' in p0:
                p1 = int(p0.split('LISTENING')[1].strip().split()[0])  # 获取进程号
                os.popen(f'taskkill /F /PID {p1}')  # 结束进程
                print(f'Appium server on port {post_num} 已结束')
        elif pc.upper() == 'MAC':
...
def start_pcapdroid_session(device_name):
    ...
    options.system_port = 8220
    options.platform_version = '12'
    options.mjpeg_server_port=9090
...
def start_pcapdroid_session2(device_name):
    options = UiAutomator2Options()
    options.platform_name = 'Android'
    options.deviceName = device_name
    options.udid = device_name
    options.platform_version = '12'
    options.system_port=8224
    options.mjpeg_server_port = 9091
    options.app_activity = 'com.emanuelef.remote_capture.activities.MainActivity'
    options.app_package = 'com.emanuelef.remote_capture'
    options.no_reset = True
    options.dont_stop_app_on_reset = True  # 防止重置和关闭应用
    ...
    # 初始化 WebDriver
    return webdriver.Remote(command_executor='http://localhost:4726', options=options)
...
# 执行操作步骤
def operation_steps(driver,app_name,waite_time):
    for app in app_name:
        # 第一步:点击PCAP文件选项
...
def execute_maxim(adb_command,adb_command2, adb_command3):
    stop_appium([4723, 4726])# 先判断端口是否被占用，如果被占用则关闭该端口号
...
def run_appium_session(device_name, command_executor, device_id,app_name):
    # 这里可以调用执行操作步骤的函数
    driver = start_pcapdroid_session(device_id)
    operation_steps(driver, app_name, 2)
    # 例如: operation_steps(driver, app_name, waite_time)
    # 完成后关闭 WebDriver
    driver.quit()
...
if __name__ == '__main__':
    ...
    #填写设备名称
    thread1 = threading.Thread(target=run_appium_session,
                               args=('127.0.0.1:62025', 'http://localhost:4723', '127.0.0.1:62025',app_name))
    thread2 = threading.Thread(target=run_appium_session,
                               args=('127.0.0.1:62026', 'http://localhost:4726', '127.0.0.1:62026',app_name2))
